...
void __fastcall PlayLayer_update_H(PlayLayer* self, void*, float unk) {
    ...
    if (fromOffset<bool>(self, 0x461)) {
        beatScale = fromOffset<float>(FMODAudioEngine::sharedEngine(), 0x110);
    } else {
        beatScale = fromOffset<float>(fromOffset<void*>(self, 0x380), 0x1B0) * 3.0f; // From my experience this is needed
    }

    if (beatScale > 0.5f && self->getActionByTag(1001) == nullptr && pulseEnabled) {
        CCScaleTo* scaleTo = CCScaleTo::create(0.1f, 1.0f + (beatScale - 0.5f) * 0.4f);
        scaleTo->setTag(1001);
